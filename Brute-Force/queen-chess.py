...
def board_nr_queens(board):
	counter = 0
	length = len(board)
	for i in range(length):
		for j in range(length):
			pos = (i,j)
			if(get_pos_value( pos, board ) == 'Q'):
				counter += 1
	return counter
...
class queen_chess_state(object):
	def __init__(self, board):
		self.board = board
		self.queens = board_nr_queens(board)
...
class queen_chess(Problem):
	...
	'''def h(self, node):
		A state is closer to the solution if ...
...
if(len(sys.argv) < 2):
	print('number of queens not specified.')
	sys.exit()
else:
